...
public interface IVoucherRepo extends JpaRepository<Voucher, Long> {
    ...
    @Query(value = "select * from voucher where id_txn_created_by in (select id from txn where id_payer = (select id from customer where ocb_user =:ocbUser) and id_use_case = 174)", nativeQuery = true)
    List<Voucher> findAllByOcbUser(String ocbUser);
}
